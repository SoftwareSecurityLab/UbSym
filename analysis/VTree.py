...
class _VTree(angr.Analysis):
    def __init__(self,cfg_analyzer=None,solo=True,buf_symbolic_bytes=100, max_str_len=100):
        global BUF_SYMBOLIC_BYTES, MAX_STR_LEN
        ...
        self.PointerNum = 0
        self.WFreeaddrs = {}
        BUF_SYMBOLIC_BYTES = buf_symbolic_bytes
    ...
    def _collect(self,state,status,VulnType,extra_const=None):
        ...
        if len(self._graph.nodes) == 0:
            ...
            if (VulnType  < 2) and self._isInCallableBoundry(self._func,state.addr) == False:
                return
            if (VulnType >= 2) and self.project._sim_procedures.get(state.addr, None ):
                return
